...
async function getDisplayData(code: string) {
  ...
  // Merge display_settings with defaults
...
export default async function DisplayPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  // Build views list based on settings
                ...
                {weeklyTopic && (
                  <Card className="rounded-[2rem] border-0 bg-white shadow-xl mb-4">
                    <CardContent className="p-6 flex items-center gap-6">
                      <Badge className="rounded-full px-5 py-2 text-xl font-semibold text-white whitespace-nowrap" style={{ backgroundColor: department.color }}>נושא השבוע</Badge>
                      <div>
